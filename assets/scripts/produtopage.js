        ...
        .then((SnapShotUser) => {
          SnapShotUser.forEach((docUser) => {
            ...
            db.collection("Categorias")
              .where("doc_ID", "==", prodVisibility.prod_id_categ_edit)
              .get()
              .then((SnapShotCateg) => {
                SnapShotCateg.forEach((docCateg) => {
                  ProdData.innerHTML = `  <h3>${docCateg.data().nome_categ} ${prodVisibility.prod_nome_edit
                    } ${prodVisibility.prod_marca_edit}</h3>
                        ...
                        <h6 id="Quantidade"></h6>
                        <div id="AllTams"></div>
                        <button onclick="SetCar(this)" class="bttcomprar" id="${prodVisibility.prod_id_edit
                    }">Comprar</button>
                  ...
                  db.collection("Produtos")
                    .where("doc_ID", "==", prodVisibility.prod_id_edit)
                    .get()
                    .then((SnapShotProd) => {
                      SnapShotProd.forEach((docProd) => {
                        docProd.data().tamanhos.forEach((docProdTam) => {
                          allestoque.push(docProdTam.quantidade);
                          let quantidadeh6 =
                        ...
                        let idBtt = document.getElementById(
                          prodVisibility.prod_id_edit
                        );
                        let faltaestoquebtt =
